...
class Products extends Component{
    ...
    forBigScreen(){
        return (
            ...
            // </>
            <>
                {<div className="row row-cols-3 text-center mx-3 my-5">
                    {this.productItems.map((item, itemIndex)=>{
                        return (
                            <Container key={itemIndex}>
                                <Image rounded={true} src={`${require(`../assets/images/${item}`)}`} style={{ height: '95%', width: '95%'}}></Image>
                            </Container>
                        )
                    })}
                </div>}
